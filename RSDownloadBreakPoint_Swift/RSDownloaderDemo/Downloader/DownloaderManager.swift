 ...
 */
let DownloadMaxTaskCount=3


class DownloaderManager: NSObject {

    static var shared:DownloaderManager=DownloaderManager()
    ...
    override init() {
        ...
        notificationCenter .addObserver(self, selector: #selector(downloadTaskDidFinishDownloading(sender:)), name: NSNotification.Name(rawValue: DownloadTaskDidFinishDownloadingNotification), object: nil)
        ...
        notificationCenter.addObserver(self, selector: #selector(systemInsufficientSpace(sender:)), name: NSNotification.Name(rawValue: DownloaderInsufficientSystemFreeSpaceNotification), object: nil)
    }
    ...
    func cancelAllTasks() {
        ...
        self.taskDict?.enumerateKeysAndObjects({ (key, obj, stop) in
            ...
            let downloader:Downloader=obj as! Downloader
            downloader.cancel()
    ...
    func cancelDownloadTaskWithUrlString(urlString:String) {
        ...
        let downloader:Downloader?=self.taskDict?.object(forKey: urlString) as! Downloader?
        downloader?.cancel()
        self.sychronized(lock: self) {
    ...
    func lastProgress(urlString:String) ->Float {
        return Downloader.lastProgress(url: urlString)
    }
    
    func filesSize(urlString:String) -> String {
        return Downloader.filesSize(url: urlString)
    }
}
